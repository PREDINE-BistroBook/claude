...
2. **Locali & Ordinazioni** — smart websites + ordering systems built and operated for bar/restaurant clients. Revenue here is B2B: client sites, not individual diners. **No clients sold yet as of 2026-09-11** — Sergio Bar and Carrozze are demo/prototype sites, never sold to a real business, despite earlier docs in this repo implying otherwise. They still exist as live Cloudflare Workers (Ash chose to keep the infrastructure running, just stop tracking them as if they were client incidents) and still consume real Cloudflare account resources — notably, they shared the account's Cloudflare D1 free-tier daily read quota with amicomiotour.com, and their larger databases could exhaust it first, breaking Tour's own `/admin` dashboard (see Site Ops incident history, 2026-09-02 through 09-11). **Resolved 2026-09-12: Ash upgraded the Cloudflare account to the paid Workers plan**, which replaces the 5M-reads-per-day cap with a monthly allowance far above what all sites combined use. Treat any new `/admin` failure as a fresh problem, not the old quota one.
