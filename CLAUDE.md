...
**A live alternative — Roberto's chat window (added 2026-09-02):** a published Artifact (Roberto persona, Head Chef) gives Ash a real live chat, separate from Slack — see the entry in the roster above. It has no live line into Notion/Slack/Stripe/Gmail itself, but it can queue a request for a specific agent: Ash tells Roberto what he wants and who it's for, Roberto logs it to the artifact's own routing queue, and a Routine ("Roberto's router — deliver queued requests," hourly, on the hour) posts it into that agent's Slack channel and fires an on-demand run where one exists. The router only relays Ash's own words — it never decides, drafts, or approves anything, so every Mode/approval rule below still applies exactly as if Ash had posted in that agent's channel directly. Delivery takes up to an hour, not instant; say so if asked.
