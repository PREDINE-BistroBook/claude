  ...
  const Demo = {
    handle(method, path, body) {
      ...
      if (path.startsWith("/api/admin/")) {
        ...
        const myT = therapists.find((x) => x.admin_id === a.id); const canSee = (b) => a.role === "all" || !b.therapist_id || b.therapist_id === myT?.id;
        { const acc = path.match(/^\/api\/admin\/bookings\/(\w+)\/accept$/); if (acc && method === "POST") { if (!myT) err("Your account isn't linked to a therapist profile. Ask the owner to link it under Team."); const bk = bookings.find((x) => x.id === acc[1]); if (!bk || !canSee(bk)) err("Not found", 404); if (bk.therapist_id && bk.therapist_id !== myT.id) err("Someone else already took this booking.", 409); bk.therapist_id = myT.id; return { ok: true, therapist: myT.name }; } }
        ...
        if (path === "/api/admin/bookings" && method === "GET") {
          let list = bookings.filter((b) => inScope(b) && b.status !== "pending" && canSee(b));
          ...
          return { bookings: list.slice(0, 300).map((b) => { const u = users.find((x) => x.id === b.user_id); const i = u?.intake || {}; return { ...b, therapist: therapists.find((t) => t.id === b.therapist_id)?.name || null, platform_fee: a.role === "all" ? b.platform_fee : undefined, health: i.health || [], flags: (i.health || []).filter((h) => FL.includes(h)), pain: i.pain || [], goals: i.goals || [], experience: i.experience || null, user_notes: u?.notes || "", user_nearest: u?.nearest_city || null }; }), city };
        }
        ...
        if (mm && method === "PATCH") { const b = bookings.find((x) => x.id === mm[1] && inScope(x) && canSee(x)); if (!b) err("Not found", 404); if (body.therapist_id !== undefined) { const want = body.therapist_id || null; if (a.role === "all") b.therapist_id = want; else if (!(want === b.therapist_id || (!b.therapist_id && myT && want === myT.id))) err("Only the owner can assign a booking to someone else.", 403); else b.therapist_id = want; } if (body.status) b.status = body.status; if (body.slot !== undefined) b.slot = body.slot; if (body.note !== undefined) b.note = body.note; if (body.therapist_note !== undefined) b.therapist_note = body.therapist_note; return { ok: true }; }
