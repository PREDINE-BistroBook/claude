...
(() => {
  ...
  // 2026-09-14: an invite link from a therapist who trained them (?ref=code) puts them on that captain's team; "somewhere else" asks for the city
  const REF = new URLSearchParams(location.search).get("ref") || "";
  const CITY_LABEL = { cairo: "Cairo", dahab: "Dahab", florence: "Florence" };
  if (REF) fetch("/api/captain/" + encodeURIComponent(REF)).then((r) => (r.ok ? r.json() : null)).then((d) => { if (!d?.captain) return; const el = $("#j-captain"); el.textContent = t("You're applying to join {name}'s team ({city}). If they trained you, a share of what you make through Zen goes to them, the way you agreed between you.", { name: d.captain.name, city: t(CITY_LABEL[d.captain.city] || d.captain.city) }); el.hidden = false; if (CITY_LABEL[d.captain.city]) $("#j-city").value = d.captain.city; }).catch(() => {});
  $("#j-city").addEventListener("change", () => { $("#j-city-other").hidden = $("#j-city").value !== "other"; });
  $("#join-form").addEventListener("submit", async (e) => {
    ...
    if (v("#j-city") === "other" && !v("#j-city-text")) { err.textContent = t("Tell us which city."); err.hidden = false; $("#j-city-text").focus(); return; }
    ...
    try {
      const r = await fetch("/api/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ref: REF, city_text: v("#j-city-text"), name: v("#j-name"), email: v("#j-email"), phone: v("#j-phone"), city: v("#j-city"), title: v("#j-title"), languages: v("#j-langs"), area: v("#j-area"), address: v("#j-address"), maps_url: v("#j-maps"), instagram: v("#j-insta"), bio: v("#j-bio"), story: v("#j-story"), certs: v("#j-certs"), photo, lat: pin?.lat ?? null, lng: pin?.lng ?? null, radius_km: Number($("#j-radius").value) || 0, agree: true, lang: I.lang }) });
