...
function card(th, i) {
  ...
  return `<article class="tm-feature ${i % 2 ? "flip" : ""}" data-city="${esc(th.city)}" data-id="${esc(th.id)}">
    ...
    <div class="tm-body">
      ...
      ${th.captain_name ? `<p class="tm-meta tm-trained">${t("Trained by {name}", { name: esc(th.captain_name) })}</p>` : ""}
      ${th.rating_n || th.sessions_done ? `<p class="tm-meta tm-brief">${th.rating_n ? `<span class="tm-stars">★ ${Number(th.rating_avg).toFixed(1)}</span><span>${t("{n} ratings", { n: th.rating_n })}</span>` : ""}${th.sessions_done ? `<span>${t("{n} sessions", { n: th.sessions_done })}</span>` : ""}</p>` : ""}
