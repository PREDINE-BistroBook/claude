...
/* ---------- gift a session ---------- */
function giftServices() { const c = CITIES[$("#g-city").value]; $("#g-service").innerHTML = c.services.map(s => `<option value="${esc(s.id)}">${esc((I.lang !== "en" && s.i18n && s.i18n[I.lang] && s.i18n[I.lang].name) || t(s.name))} · ${s.dur} ${t("min")} — ${money(s.price, c.currency)}</option>`).join(""); giftPrice(); }
...
$("#gift-form").addEventListener("submit", async (e) => {
  ...
  const btn = $("#g-pay"), label = btn.textContent; btn.disabled = true; btn.setAttribute("aria-busy", "true"); btn.textContent = t("One moment…");
  try {
