...
async function loadTeam() {
  ...
  // a position saved on the "Near you" page (or the account) sorts by real distance; otherwise the profile's neighbourhood text
  ...
  const kmTo = th => (LOC && th.lat !== null && th.lat !== undefined && th.lng !== null && th.lng !== undefined) ? (() => { const R = 6371, toR = x => x * Math.PI / 180, dLat = toR(th.lat - LOC.lat), dLng = toR(th.lng - LOC.lng), h = Math.sin(dLat / 2) ** 2 + Math.cos(toR(LOC.lat)) * Math.cos(toR(th.lat)) * Math.sin(dLng / 2) ** 2; return 2 * R * Math.asin(Math.sqrt(h)); })() : null;
