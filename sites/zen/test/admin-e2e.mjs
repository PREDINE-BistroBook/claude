...
// 2026-09-14: the owner (has an email) sees no nudge; the client drawer carries the pain history; statements are team totals
ok("email: no nudge for a sign-in that has an email", await page.isHidden("#email-nudge"));
ok("nudges: a strong password gets no warning", await page.isHidden("#weak-nudge"));
ok("nudges: the owner is told card payments are off and exactly what's missing", await page.isVisible("#pay-nudge") && /Missing: Zen's Stripe account id, the Stripe secret key, the Stripe webhook secret/.test(await page.textContent("#pay-nudge")) && /Fawry is off/.test(await page.textContent("#pay-nudge")), await page.textContent("#pay-nudge"));
ok("errors: the box stays hidden while nothing was reported", await page.isHidden("#errors-box"));
await fetch(H + "/api/log", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ page: "/booking?city=dahab", msg: "ReferenceError: foo is not defined @ booking.js:99" }) });
await page.goto(H + "/admin.html", { waitUntil: "load" }); await page.waitForTimeout(1000); ok("errors: a reported browser error shows on the owner's overview with the page and the message", await page.isVisible("#errors-box") && /booking\?city=dahab/.test(await page.textContent("#errors-list")) && /ReferenceError/.test(await page.textContent("#errors-list")), await page.textContent("#errors-list").catch(() => ""));
{ const r1 = await fetch(H + "/api/auth/request-link", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: "painpat@x.com", name: "Pain Pat" }) }); const link = (await r1.json()).link; const r2 = await fetch(link, { redirect: "manual" }); const ck = (r2.headers.getSetCookie?.() || [r2.headers.get("set-cookie")]).map((c) => c.split(";")[0]).join("; ");
  await fetch(H + "/api/me", { method: "PUT", headers: { "content-type": "application/json", cookie: ck }, body: JSON.stringify({ country: "EG", nearest_city: "cairo", intake: { goals: ["pain"], pain: ["lower_back", "neck"], health: [], completed_at: "2026-09-14" } }) });
  await fetch(H + "/api/me", { method: "PUT", headers: { "content-type": "application/json", cookie: ck }, body: JSON.stringify({ intake: { goals: ["pain"], pain: ["neck"], health: [], completed_at: "2026-09-14" } }) });
  await page.fill("#q", "painpat"); await page.waitForTimeout(700); await page.click("#q-results [data-client]"); await page.waitForTimeout(800);
  ok("drawer: the client's pain history shows as a strip of mini bodies, first answers → their update, with the change spelled out", (await page.$$("#drawer .pain-strip .pain-pt")).length === 2 && /first answers/.test(await page.textContent("#drawer .pain-strip")) && /Lower back/.test(await page.textContent("#drawer")), (await page.textContent("#drawer")).slice(0, 400)); await page.click("#dr-close").catch(() => {}); await page.waitForTimeout(300); }
await tab("money"); ok("money: the owner's tab reads Statements with totals per city, and the help explains who sees what", (await page.textContent("#money-title")).trim() === "Statements" && /Who sees what/.test(await page.textContent("#tab-money")) && !/Adham|Hesham/.test(await page.textContent("#st-list")), (await page.textContent("#st-list")).slice(0, 200));
...
{ const ym = new Date().toISOString().slice(0, 7), day = ym + "-15";
  ...
  const card = page.locator("#st-list .statement", { hasText: "Cairo · team" }); ok("money: one 'Cairo · team' card for the owner (never Adham's own line): 2 sessions, cash EGP 1,800, nothing to pay from the site", (await card.count()) === 1 && (await page.locator("#st-list .statement", { hasText: "Adham" }).count()) === 0 && /Cash[\s\S]*1,800/.test(await card.textContent()) && /To pay the team[\s\S]*EGP.0\b/.test(await card.textContent()) && /Sessions[\s\S]*2/.test(await card.textContent()), await card.textContent().catch(() => ""));
  ok("money: the session table has no therapist column for the owner", !/Adham/.test(await page.textContent("#st-table")), (await page.textContent("#st-table")).slice(0, 200));
...
await page.screenshot({ path: process.env.S + "/shot-achat.png" }); await page.setViewportSize({ width: 390, height: 844 }); await page.waitForTimeout(400); await page.screenshot({ path: process.env.S + "/shot-achat-m.png" }); await page.setViewportSize({ width: 1280, height: 900 }); await page.waitForTimeout(300);
...
// first a username-only sign-in for Anas: the owner's card flags "no email yet", and Anas gets the red nudge until he adds one
await tab("team"); await page.locator("#team-list .team-card", { hasText: "Anas" }).click(); await page.waitForTimeout(400);
await page.selectOption("#th-admin", "new"); await page.waitForTimeout(200); await page.fill("#th-new-email", "anas"); await page.fill("#th-new-pass", "Anas-pass-12345"); await page.click("#th-save"); await page.waitForTimeout(1200); await page.click("#ask-yes").catch(() => {}); await page.waitForTimeout(600);
ok("email: the owner's team card says Anas has no email yet, so nothing can reach him", /signs in as.*anas.*no email yet/.test((await page.locator("#team-list .team-card", { hasText: "Anas" }).textContent()).replace(/\s+/g, " ")), (await page.locator("#team-list .team-card", { hasText: "Anas" }).textContent()).replace(/\s+/g, " ").slice(0, 300));
...
await page.fill("#a-email", "anas"); await page.fill("#a-pass", "Anas-pass-12345"); await page.click("#login-btn"); await page.waitForTimeout(1500);
ok("nudges: Anas's dashed password is not flagged; the pay nudge is for the owner and the platform only", await page.isHidden("#weak-nudge") && await page.isHidden("#pay-nudge"));
ok("email: Anas sees the red 'Add your email' nudge on every screen", await page.isVisible("#email-nudge") && /Add your email/.test(await page.textContent("#email-nudge")), await page.textContent("#email-nudge").catch(() => ""));
await page.click("#email-nudge-go"); await page.waitForTimeout(500); ok("email: its link lands on Settings", await page.isVisible("#tab-settings"));
{ const r = await page.evaluate(async () => (await fetch("/api/admin/profile", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: "anas@x.com" }) })).json()); ok("email: he adds his email under his profile", r.admin?.email === "anas@x.com", r);
...
// sign-ins on/off is the platform account's (2026-09-15, Ash): the owner's table shows the badge only
await tab("settings"); ok("sign-ins: the owner's table has no Switch off button", (await page.$$("#admins-table [data-off], #admins-table [data-sw]")).length === 0 && (await page.$$("#admins-table [data-del]")).length >= 1);
await page.evaluate(() => fetch("/api/admin/logout", { method: "POST" })); await page.goto(H + "/admin.html", { waitUntil: "load" }); await page.waitForTimeout(600);
...
await tab("services"); await page.waitForTimeout(500); ok("employee: the Services tab is there, with 'Add one of your own' and the city's list read-only", await page.isVisible("#tab-services") && (await page.textContent("#add-service")).includes("your own") && (await page.$$("#services-table tbody tr")).length >= 3 && (await page.$$("#services-table [data-edit]")).length === 0, await page.textContent("#add-service"));
await page.click("#add-service"); await page.waitForTimeout(400); await page.fill("#sv-name", "Hesham's neck reset"); await page.fill("#sv-min", "40"); await page.fill("#sv-amount", "850"); await page.fill("#sv-desc", "Neck and shoulders, cups and hands."); await page.click("#svc-form button[type=submit]"); await page.waitForTimeout(1200);
{ const row = page.locator("#services-table tbody tr", { hasText: "neck reset" }); ok("employee: his own service is listed as his, editable", (await row.count()) === 1 && (await row.textContent()).includes("own") && (await row.locator("[data-edit]").count()) === 1, await page.textContent("#services-table"));
...
await tab("team"); ok("owner: the team list is Florence only (Shika), the Egyptian cards are gone", /Shika/.test(await page.textContent("#team-list")) && !/Hesham|Adham|Mazen|Anas|Shaarawy/.test(await page.textContent("#team-list")), (await page.textContent("#team-list")).slice(0, 160));
{ const bl = await (await page.request.get(H + "/api/admin/bookings")).json(); ok("owner: the booking list has no Cairo or Dahab row while they're closed (the seeded ones are all Egyptian, so it is empty)", bl.bookings.every((b) => b.city === "florence"), bl.bookings.map((b) => b.city).slice(0, 10)); }
