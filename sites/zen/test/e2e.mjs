...
{ const mk = await page.evaluate(async () => (await (await fetch("/api/checkout", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ city: "cairo", service: "cai-dry", date: "2027-05-05", slot: "morning", name: "Sara Tester", email: window.__em, phone: "+20 111", flagged: true, agreed: true, lang: "en" }) })).json()));
  ok("account: a review-path booking exists for the client", mk && (mk.url || "").includes("review"), mk);
  await go("account.html"); await page.waitForTimeout(600); if (await page.isVisible("#wizard")) { await page.evaluate(() => { document.querySelector("#wizard").hidden = true; document.querySelector("#app-view").hidden = false; }); }
  await page.click('.tabs [data-tab="sessions"], #tab-sessions-btn, button:has-text("Sessions")').catch(() => {}); await page.waitForTimeout(400);
