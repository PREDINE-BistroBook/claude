...
- **Services and prices live in the database** (`services` table), edited in the admin under **Services & prices**: add, rename, reprice, change duration, hide/show, remove. The website reads them on load (`GET /api/catalog`) and checkout charges what the table says, never what the browser sends. `src/catalog.js` is only the seed and the fallback if the table is empty. Removing a service that was ever booked hides it instead, so history stays intact. Per-city address, team line, WhatsApp, Maps and review links are in Settings → Per city.
