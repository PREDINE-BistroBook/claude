...
async function waitlist(env) {
  ...
  for (const w of rows) {
    ...
    const ok = await sendEmail(env, { to: w.email, subject: `A slot opened in ${CITIES[w.city].name} on ${w.date}`, text: [`Hi ${w.name.split(" ")[0]},`, ``, `Free times in ${CITIES[w.city].name} on ${w.date}: ${match.slice(0, 6).map((s) => s.time).join(", ")}.`, ``, `Book it before someone else does: ${env.SITE_URL}/?city=${w.city}&date=${w.date}#book`, ``, `Zen Recovery`].join("\n") });
