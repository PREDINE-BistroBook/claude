...
export async function sendStatements(env, month) {
  ...
  for (const a of admins) {
    ...
    const lines = d.rows.map((r) => `${r.therapist || r.city} · ${r.sessions} session${r.sessions === 1 ? "" : "s"} (${r.done} done, ${r.upcoming} to come${r.kept ? ", " + r.kept + " fee kept" : ""})\n  gross ${fmt(r.gross, r.currency)} · site takings ${fmt(r.site_takings, r.currency)} · platform fee ${fmt(r.fee, r.currency)} · ${mine ? "coming to you" : "to pay the team"} ${fmt(r.to_therapist, r.currency)}${r.cash_amount ? ` · collected in cash ${fmt(r.cash_amount, r.currency)}` : ""}${r.captain_due ? ` · to your captain ${r.captain}: ${fmt(r.captain_due, r.currency)}` : ""}${r.team_share ? ` · from your team: ${fmt(r.team_share, r.currency)}` : ""}${r.captain_shares ? ` · captain shares inside that: ${fmt(r.captain_shares, r.currency)} (${fmt(r.platform_on_captains, r.currency)} of it to the platform)` : ""}`);
