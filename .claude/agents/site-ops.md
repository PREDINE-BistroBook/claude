...
5. End with a digest: sites checked, alerts logged/closed, items fixed, items left in the queue and why. If nothing happened, say so explicitly — per the silence rule in `CLAUDE.md`, a digest is required every run even when everything is green.
