...
6. Before closing: this agent is closest to what performs and what doesn't — if a real content idea surfaces beyond the standard three angles (a trend, a format worth trying, something a competitor's doing), log it as an Idea per `CLAUDE.md` instead of just letting it pass. Only when there's something real to say.
