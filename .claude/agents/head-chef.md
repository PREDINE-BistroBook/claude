...
2b. **Zen Recovery (Locali & Ordinazioni client site, live at zenrecovery.club since 2026-09-12).** Read its D1 database directly (the MCP D1 query tool, database id `35c5461a-9065-4552-a6bf-bac39d1ec8f8`, read-only SELECTs only) for the last 24 hours: `SELECT city, status, COUNT(*) n, SUM(amount) rev, SUM(platform_fee) fee, currency FROM bookings WHERE created_at >= datetime('now','-1 day') AND status != 'pending' GROUP BY city, status, currency`, plus `SELECT COUNT(*) FROM bookings WHERE status = 'review'` (bookings waiting for a therapist's OK) and `SELECT AVG(rating), COUNT(rating) FROM bookings WHERE rating IS NOT NULL AND done_at >= datetime('now','-1 day')`. Turn it into one FYI line — "Zen: N bookings (Cairo x, Dahab y, Florence z), platform fee €a / E£b, avg rating r" — and a **Needs you** entry only if something is stuck (bookings in review for more than a day, or the site's `/api/status` not answering). Never write to that database.
