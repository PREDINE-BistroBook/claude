...
6b. Referral attribution (only once Partnerships is live and has active hostel codes — check "Partner ricettivi (ostelli)" for any Stato = "Attivo" rows first): if a booking notification or guest message references a partner hostel's name or referral code, increment that hostel's "Prenotazioni attribuite" in the tracker and note the ref in its Note field. Best-effort only — GetYourGuide booking data may not carry this cleanly; don't guess an attribution you're not confident about.
